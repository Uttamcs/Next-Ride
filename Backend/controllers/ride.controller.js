...
module.exports.requestRide = async (req, res) => {
  try {
    ...
    const { pickupLocation, dropLocation, distance, duration, vehicleType, paymentMethod } = req.body;
...
module.exports.cancelRide = async (req, res) => {
  try {
    ...
    if (cancelledBy === 'captain') {
      await captainModel.findByIdAndUpdate(req.captain._id, { isAvailable: true });
    }
    
    // Cancel the ride
    const ride = await rideService.cancelRide(rideId, cancelledBy, cancellationReason);
...
module.exports.getCaptainRideHistory = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error getting captain ride history', error: error.message });
  }
};

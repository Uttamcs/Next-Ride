...
router.post(
  ...
  [
    ...
    body('pickupLocation.coordinates.longitude').isFloat().withMessage('Pickup longitude must be a valid number'),
    body('pickupLocation.coordinates.latitude').isFloat().withMessage('Pickup latitude must be a valid number'),
    body('dropLocation.address').notEmpty().withMessage('Drop address is required'),
    body('dropLocation.coordinates.longitude').isFloat().withMessage('Drop longitude must be a valid number'),
    body('dropLocation.coordinates.latitude').isFloat().withMessage('Drop latitude must be a valid number'),
    ...
    body('paymentMethod').optional().isIn(['cash', 'card', 'wallet']).withMessage('Payment method must be cash, card, or wallet')
  ],
